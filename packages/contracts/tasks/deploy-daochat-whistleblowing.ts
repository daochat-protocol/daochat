...
task("deploy:daochat-whistleblowing", "Deploy a DaochatWhistleblowing contract")
    ...
    .setAction(async ({ logs }, { ethers }): Promise<any> => {
        ...
        const daochatWhistleblowingFactory = await ethers.getContractFactory("DaochatWhistleblowing", {
            libraries: {
                IncrementalBinaryTree: incrementalBinaryTree.address
            }
        })

        const daochatWhistleblowing = await DaochatWhistleblowingFactory.deploy(daochatVerifier.address)

        await daochatWhistleblowing.deployed()

        if (logs) {
            console.info(`DaochatWhistleblowing contract has been deployed to: ${daochatWhistleblowing.address}`)
        }

        return {
            daochatWhistleblowing,
